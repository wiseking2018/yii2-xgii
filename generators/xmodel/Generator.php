...
namespace yii\xgii\generators\xmodel;
 ...
 * @since 2.0
 */
class Generator extends \yii\xgii\Generator
{
    const RELATIONS_NONE = 'none';
    const RELATIONS_ALL = 'all';
    const RELATIONS_ALL_INVERSE = 'all-inverse';

    const TABLE_CHARACTER_SET_UFT8 = 'utf8_unicode_ci';
    const TABLE_CHARACTER_SET_GBK= 'gbk_chinese_ci';
    const TABLE_ENGINE_INNODB = 'InnoDB';
    const TABLE_ENGINE_MYISAM = 'MyISAM';
    const TABLE_ENGINE_MEMORY = 'MEMORY';

    const CREATE_TABLE_FLAG_SUCCESS = 1;
    const CREATE_TABLE_FLAG_FAILURE = 0;
    ...
    {
        return '基于表单填写的数据表名称以及字段，自动生成模型代码文件，同时在数据库中创建相同构架的数据表。';
    }
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            ...
            [['db', 'ns', 'tableName','tableCharacter','tableEngine','tableStructure', 'baseClass', 'queryNs', 'queryBaseClass'], 'required'],
    ...
    public function attributeLabels()
    {
        return array_merge(parent::attributeLabels(), [
            ...
            'db' => '数据库连接ID',
            'tableName' => '数据表名称',
            'tableCharacter' => '数据表字符集',
            'tableEngine' => '数据表引擎',
            'tableStructure' => '数据表架构（字段定义）',
            'modelClass' => '模型（Model）类名称',
            'baseClass' => '基类（Base Class）',
            'useTablePrefix' => '使用表前缀',
            'generateRelations' => '生成表间关系',
            'generateRelationsFromCurrentSchema' => '根据当前架构生成表间关系',
            'generateLabelsFromComments' => '根据数据表备注生成标签名称',
            'generateQuery' => '生成ActiveQuery查询代码',
            'queryNs' => 'ActiveQuery查询代码命名空间',
            'queryClass' => '查询代码类名',
            'queryBaseClass' => 'ActiveQuery查询代码基类',
            'enableI18N' => '启用I18N国际化',
            'useSchemaName' => '使用架构名称',
        ]);
    }
    ...
    {
        // 'createDataTable','tableStructor',
        return array_merge(parent::stickyAttributes(), ['ns', 'db','baseClass','tableCharacter','tableEngine', 'generateRelations', 'generateLabelsFromComments', 'queryNs', 'queryBaseClass', 'useTablePrefix', 'generateQuery']);
    }
    ...
    public function validateTableName()
    {
        ...
        if (strpos($this->tableName, '*') !== false && substr_compare($this->tableName, '*', -1, 1)) {
            $this->addError('tableName', '星号只允许结尾使用');
            return;
        }
        //生成数据表构架
        $columns = [];
        if (isset($this->tableStructure)) {
            $columns = $this->array_from_string($this->tableStructure);
        }
        if(empty($columns)) {
            $this->addError('tableStructure', '数据表结构不正确');
            return;
        }
        //执行建表操作
        $db = $this->getDbConnection();
        if ($db === null) {
            $this->addError('tableStructure', '数据库连接不成功');
            return;
        }
        $this->dropDataTable($db, $this->tableName);

        $charSet = 'utf8 COLLATE utf8_unicode_ci';
        if ($this->tableCharacter === self::TABLE_CHARACTER_SET_GBK) {
            $charSet = 'gbk COLLATE gbk_chinese_ci';
        }
        $tableOptions = 'CHARACTER SET '.$charSet.' ENGINE='.$this->tableEngine;

        $db->createCommand()->createTable($this->tableName,$columns,$tableOptions)->execute();

        //遍历库中数据表并判断是否存在
        $tables = $this->getTableNames();
        if (empty($tables)) {
            $this->addError('tableName', "Table '{$this->tableName}' 数据表创建失败");
        } else {
            foreach ($tables as $table) {
                $class = $this->generateClassName($table);
                if ($this->isReservedKeyword($class)) {
                    $this->addError('tableName', "Table '$table'将产生含有PHP保留字符名称的数据表.");
                    break;
                }
            }
        }
    ...
    public function checkTableName() {
        ...
        //保留字段
        $class = $this->generateClassName($this->tableName);
        if ($this->isReservedKeyword($class)) {
            $this->addError('tableName', "Table '$table'将产生含有PHP保留字符名称的数据表.");
        }
    }
    ...
    protected function createDbTable($overWrite = true)
    {
        $db = $this->getDbConnection();
        if ($db === null) {
            return 0;
        }
        if ($this->tableExisted($db, $this->tableName)) {
            $db->createCommand()->dropTable($this->tableName)->execute();
        }
        $options = array($this->tableStructor);
//        $options = [
//            'id' => 'pk',
//            'name' => 'string',
//            'age' => 'integer',
//        ];
        if ($db->createCommand()->createTable($this->tableName, $options)->execute()) {
            return 1;
        }
        else {
            return 0;
    ...
    protected function dropDataTable($db, $tableName) {
        $db = $this->getDbConnection();
        if ($db === null || !isset($tableName)) {
            return;
        }
        if ($this->tableExisted($db, $tableName)) {
            $db->createCommand()->dropTable($tableName)->execute();
        }
    }

    protected function tableExisted($db, $tableName) {
        if ($db === null) {
            return true;
        }
    ...
    public function array_from_string($array_str) {
        //$array_str = 'id=>pk,name=>string,age=>integer';
        if (!isset($array_str)) {
            return [];
        }
        $array_dest = [];
        $columns_array = explode(',',$array_str);
        $arr_t = [];
        foreach ($columns_array as $v) {
            $temp_arr = explode('=>',$v);
            if (count($temp_arr) == 2) {
                $arr_t = [
                    trim($temp_arr[0]) => trim($temp_arr[1])
                ];
                $array_dest = ArrayHelper::merge($array_dest,$arr_t);
            }
        }
        return $array_dest;
    }
    public function string_2_array($array) {
        $str = '';
        foreach($array_dest as $name=>$value)
        {
            $str.=$name . '=>' . $value . ',';
        }
        return rtrim($str,',');
